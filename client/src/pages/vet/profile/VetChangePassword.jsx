...
const notificationOptions = [
  { key: "appointments", label: "Appointment requests", description: "New, accepted, completed, and rejected appointment updates." },
  { key: "patients", label: "Patient activity", description: "Pet profile and medical record changes related to your appointments." },
  { key: "reviews", label: "Review alerts", description: "Owner ratings and feedback on completed consultations." },
  { key: "system", label: "System updates", description: "Account, approval, and platform notifications." },
];
...
const VetChangePassword = () => {
  ...
  const [notificationPrefs, setNotificationPrefs] = useState(() => {
    const saved = JSON.parse(localStorage.getItem("vetNotificationPrefs") || "{}");
    return {
      appointments: saved.appointments ?? true,
      patients: saved.patients ?? true,
      reviews: saved.reviews ?? true,
      system: saved.system ?? true,
    };
  });
  ...
  const toggleNotification = (key) => {
    setNotificationPrefs((current) => {
      ...
      localStorage.setItem("vetNotificationPrefs", JSON.stringify(next));
  ...
  return (
    <main>
      <VetPageHeader title="Settings" description="Manage account security, notifications, and session controls." />
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-2xl border border-white/10 bg-slate-900 p-5 shadow-lg shadow-black/10">
          <SectionHeading icon={KeyRound} title="Change Password" description="Use a strong password to keep your veterinarian account protected." />
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-900 p-5 shadow-lg shadow-black/10">
          <SectionHeading icon={LogOut} title="Session" description="Sign out from this veterinarian portal session." />
          <div className="rounded-xl border border-white/10 bg-slate-950 p-4">
            <p className="text-sm font-semibold text-white">Current session</p>
            <p className="mt-1 text-sm text-slate-500">You are signed in to the veterinarian panel on this browser.</p>
          </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-900 p-5 shadow-lg shadow-black/10 xl:col-span-2">
          <SectionHeading icon={Bell} title="Notifications" description="Choose which veterinarian updates should stay visible for this browser." />
